...
class EmployeeTable:
    scripts = {
        ...
        "insert new": text(
            "insert into employee(\"first_name\", \"last_name\", \"phone\", \"company_id\", \"is_active\") values (:f_name, :l_name, :phone, :comp_id, :active)"),
        "get max id": "select MAX(\"id\") from employee",
        "select by id": text("select * from employee where id =:select_id"),
        "select by company id": text(
            "select * from employee where company_id =:select_id"),
        "delete by company id": text(
            "delete from employee where company_id =:id_to_delete")
    }
...
# Создание сотрудника
    def create_employee(
            self, first_name, last_name, company_id, phone, is_active):
        self.db.execute(
            self.scripts["insert new"], f_name=first_name,
            l_name=last_name, phone=phone, comp_id=company_id, active=is_active
            )
...
# Получение сотрудников по ID компании
    def get_employees_by_company_id(self, id):
        return self.db.execute(
            self.scripts["select by company id"], select_id=id).\
            fetchall()
