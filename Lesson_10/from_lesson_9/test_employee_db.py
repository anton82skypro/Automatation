    ...
    with allure.step("Создать сотрудника"):
        ...
        with allure.step("Вызвать API-метод для создания"):
            new_emp = api_emp.create_employee(
                f_Name, l_Name, id_com, phone, isActive)
            id_emp = new_emp["id"]
...
def test_get_employee_list():
    ...
    with allure.step("Создать сотрудника"):
        ...
        with allure.step("Добавить сотрудника в БД"):
            ...
            api_result = api_emp.get_employee_list(
                params_to_add={'company': company_id})

        with allure.step("Проверить,что списки из БД и API равны"):
            assert len(db_result) == len(api_result)

        with allure.step("Проверить, что список сотрудников = 1"):
...
@allure.feature("Управление информацией сотрудника")
@allure.severity(allure.severity_level.NORMAL)
